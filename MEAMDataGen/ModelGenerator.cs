...
namespace MEAMDataGen
{
    public static class ModelGenerator
    {
        ...
        public static MaintenancePlan CreateMaintenancePlan(List<MaintenanceObject> objects, DateTime start, DateTime end)
        {
            var result = new MaintenancePlan();

            objects.ForEach(x =>
            {
                result[x] = CreateObjectMaintenancePlan(x, start, end);
            });

            return result;
        }

        private static ObjectMaintenancePlan CreateObjectMaintenancePlan(MaintenanceObject maintenanceObject, DateTime start, DateTime end)
        {
            var result = new ObjectMaintenancePlan(maintenanceObject);
            var day = start.Date;
            while (day <= end)
            {
                result.Calendar[day] = new List<MaintenanceTask>();
